...
export default class VpcrSnapshotChildRowCmp extends LightningElement {
    ...
    setVPCRData(value) {
        ...
        //---------------
        if(this.vpcrRec.hasOwnProperty('proactiveRenewalIncentiveStatus')
        && !this.isBlank(this.vpcrRec.proactiveRenewalIncentiveStatus)){
        
        if(this.vpcrRec.proactiveRenewalIncentiveStatus.indexOf('Pending Qualification')!== -1){
          this.addClass='slds-cell-wrap boldtxt';
          this.scesReceivingCompensation='Pending Qualification';
        }else if(this.vpcrRec.proactiveRenewalIncentiveStatus.indexOf('Fully Validated')!== -1){
            this.scesReceivingCompensation=this.vpcrRec.proactiveRenewalSCEReceivingCompensation;
          if(this.vpcrRec.hasOwnProperty('opportunityOwnerId') && !this.isBlank(this.vpcrRec.opportunityOwnerId)
            && this.vpcrRec.hasOwnProperty('accountOwnerId') && !this.isBlank(this.vpcrRec.accountOwnerId)
            && this.vpcrRec.opportunityOwnerId!== this.vpcrRec.accountOwnerId){
