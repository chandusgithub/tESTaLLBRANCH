...
({
    getMembershipByMktData : function(component) {
        ...
        var action = component.get("c.getMedicalMemByMarket");
        action.setParams({
    ...
    getMedicalStateByMarket : function(component,event,currentRegion) { 
        ...
        $A.util.removeClass(spinner2, 'slds-hide');
        var action = component.get("c.getMedicalStateByMarket");
        action.setParams({
            AccountId : component.get('v.recordId'),
        ...
        action.setCallback(this,function(response){
            ...
            if(state === 'SUCCESS'){
                ...
                for (var key in memByMktStateMap) {  
                    var stateValues = memByMktStateMap[key];
               
                    var stateObject = {
                        'name': key,
                        'members': stateValues['Members__c'],
                        'surestMembers': stateValues['Surest_Market_Members__c'],
                        'unetMembers': stateValues['UNET_Market_Members__c'],
                        'umrMembers': stateValues['UMR_Market_Members__c']
                    };
                    
                    stateMap.push(stateObject);
                } 
    ...
    getMedicalUnetByMarket : function(component,event,currentState) {        
        var action = component.get("c.getMedicalUnetsByMarket");
        action.setParams({
            AccountId : component.get('v.recordId'),
