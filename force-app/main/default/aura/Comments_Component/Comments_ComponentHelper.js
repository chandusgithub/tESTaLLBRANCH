({
    createDynamicComponent:function(component, event,ProductCategoryComponent){
        ...
        var isAdminObj = component.get('v.accessObj');
        var isAdmin = isAdminObj.isSystemAdmin;
        isAdmin = (isAdmin != undefined && isAdmin != null) ? isAdmin : false;
        var adminSpecificFields = [];
        if(!isAdmin) {
           var adminSpecificFieldsVal = $A.get("$Label.c.Comments_AdminOnly_Fields"); 
           adminSpecificFields = adminSpecificFieldsVal.split(',');
        }
        var MemberShipActivityChildAttributeObj = {'memberShipActivityRecordId':component.get('v.recordId'),
                                                   ...
                                                   'readOnlyFieldsItag':adminSpecificFields,
