({
    getEBDRecords : function(component,event) {
        var spinner = component.find("userLoadingspinner");
        $A.util.removeClass(spinner, 'slds-hide');
        $A.util.addClass(spinner, 'slds-show');
        var action = component.get("c.getEBD");
        action.setParams({
        ...
        action.setCallback(this,function(response){
            ...
            if(state == "SUCCESS") {
                if(response.getReturnValue() != null) {
                    console.log('data');
                    var responseObj = response.getReturnValue().EBDList;
                    component.set('v.hasAccess',response.getReturnValue().hasAccess);
                    component.set('v.DocumentLink',response.getReturnValue().cdMap);
                    component.set('v.hasUploadAccess',response.getReturnValue().hasUploadAccess);
                    component.set('v.YearPicklist',response.getReturnValue().yearPickList);
                    if((responseObj != null && responseObj != undefined && responseObj.length > 0)) {
                        component.set('v.isEmpty', false);
                        component.set('v.EBDCount',responseObj.length);
                        component.set("v.EBDdata", responseObj);
                    } else {
    ...
    insertNewEBD:function(component,event,ebddata,isDelete){
        var spinner = component.find("userLoadingspinner");
        $A.util.removeClass(spinner, 'slds-hide');
        $A.util.addClass(spinner, 'slds-show');
        var action = component.get("c.insertEbd");
        action.setParams({
            "linkId":component.get('v.recordId'),
            "ebddata":ebddata,
            "isDelete":isDelete
            ...
            if(state == "SUCCESS") {
                if(response.getReturnValue() != null) {
                    ...
                    if((responseObj != null && responseObj != undefined && responseObj.length > 0)) {
                        ...
                        component.set('v.isEmpty', true);
                        component.set('v.EBDCount',0);
                        component.set("v.EBDdata", []);
                        if(!component.get('v.hasAccess')){
                            component.set('v.colSpan',5);
                        }else{
                            component.set('v.colSpan',6);
